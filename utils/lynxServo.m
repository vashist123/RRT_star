...
if nargin
    ...
    % convert the q of joint 6 for gripper
    if strcmp(lynx.param.hardware, 'gripper')
        qNew(end) = interp1([-15,30],[lynx.param.lowerLim(length(qNew)), lynx.param.upperLim(length(qNew))],qNew(end));
    end
    if isnan(qNew(end))
        error('Gripper input out of range. The range for gripper is [-15, 30]');
    end
